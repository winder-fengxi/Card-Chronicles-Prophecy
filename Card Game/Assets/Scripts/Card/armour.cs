...
public class armour : CardItem, IPointerDownHandler
{
    ...
    private void CheckRayToEnemy()
    {
        ...
        RaycastHit choose;
        if (Physics.Raycast(ray, out choose, 10000, LayerMask.GetMask("Folloer")))
        {
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (TryUse() == true)
                {
                    chooseFollower.Defend += 5;
                    chooseFollower.UpdateDefend();
                }
                //随从未选中
