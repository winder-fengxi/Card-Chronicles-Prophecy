...
public class Doppelganger : CardItem, IPointerDownHandler
{
    ...
    private void CheckRayToEnemy()
    {
        ...
        if (Physics.Raycast(ray, out choose, 10000, LayerMask.GetMask("Follower")))
        {
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (TryUse() == true)
                {
                    string id = chooseFollower.IdName;
                    FollowerManeger.Instance.Separation += 2;
                    Vector3 pos = FollowerManeger.Instance.CreateFollower(id);//创建随从
                    PlayEffect(pos, 0);
                    pos = FollowerManeger.Instance.CreateFollower(id);
                    PlayEffect(pos, 0);
                }
                //随从未选中
