...
public class GameConfigManager
{
    ...
    private GameConfigData followerData;//随从表
    ...
    public void Init()
    {
        ...
        // 加载随从数据
        textAsset = Resources.Load<TextAsset>("Data/follower");
        followerData = new GameConfigData(textAsset.text);
    ...
    // 获取随从行数据
    public List<Dictionary<string, string>> GetFollowerLines()
    {

        return followerData.GetLines();
    }
    ...
    //根据ID获取随从数据
    public Dictionary<string, string> GetFollowerById(string id)
    {
        return followerData.GetOneById(id);
    }
}